...
participating footprints. Use its board-space `x`/`y`, `rotation_deg`, `shape`,
`size`, `drill`, and per-copper-layer geometry; do not estimate copper extent
from package family or a different pad in the footprint. A null geometry field
is unavailable evidence, not a zero-size pad.
